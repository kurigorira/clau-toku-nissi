...
1. 日付を前日にして開く（既定で前日になっている）
   **日報のExcel（.xlsx）があれば、画面上部でファイルを選び「Excelから読み込む」**。
   外来の欄と紙の合計の欄がすべて埋まる（まだ保存はしない）。入院はExcelに無いので 2 で打つ
...
読み込むのは **Sheet1（紙の様式）** のほう。紙と同じ数字がすべてそろっているため。
セル番地は決め打ちせず、「科名」「午前」「医科合計」「介護合計」などの見出しの文字で位置を探すので、
様式に行が増えても読める（`src/nippo.php` の `nippo_from_xlsx()`）。

- 読み込んだ値も、手で打った値と同じ照合（紙の合計と合うか）を通ってから保存される
- ファイルの「9月 3日」と画面の日付が違うときは読み込まない
- 表に無い科（特養科・精神科・透析科など）の行に人数が入っていたら、黙って捨てずに止める
- **.xlsx のみ**。古い形式（.xls）は、Excelで「名前を付けて保存」→「Excel ブック（*.xlsx）」で保存し直す
