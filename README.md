...
db/
  ...
  master/               ★ここを直す
    ...
    items.csv           項目マスタ（483項目。入力396 / 合算64 / 計算23）
    config.csv          定床などの設定値
  tools/
...
src/
  ...
  nippo.php       電子カルテ日報の並び（科×時間帯→項目コード）と、紙の合計との照合・保存
  xlsx.php        .xlsx の最小限の読み取り（ZipArchive＋SimpleXML。ライブラリ不要）
...
public/
  ...
  nippo.php          電子カルテ日報の転記（外来の入力はここ）外来・医事課・管理者
...
## 電子カルテ日報の転記（毎日の集計の出発点）

電子カルテから毎日出る紙「外来患者数（科別）」「入院患者数日報」を、
`nippo.php`（ナビの「日報転記」）に **紙と同じ並び** で打ち込む。ここから各種集計が始まる。

1. 日付を前日にして開く（既定で前日になっている）
   **日報のExcelがあれば、画面上部で「外来日報（.xlsx）」「入院日報（.xlsm）」を選び「Excelから読み込む」**。
   片方だけでもよい。読んだ欄と紙の合計の欄が埋まる（まだ保存はしない）。読まなかった側は 2 で打つ
2. 科別×時間帯の来院数・新来数、再掲（リハビリ・ドック・健診・訪問診療科）、
   訪問診療科内訳、介護、入院の入退院と科別内訳を打つ。**空欄は0として保存する**
3. 紙に印字されている **医科合計（来院・新来）・介護合計・医療介護合計・各病棟の前日患者数** も打つ。
   各科の行の「合計」列は任意（打てば照合する。同じ時間帯の中で打ち間違えたときに見つかる）
4. 「照合して保存」。紙の合計と合わない欄があれば赤くなり、**保存しない**
5. 保存すると、外来のほか **病棟（入退院）と当直（訪問系・通所リハ）にも下書き** として値が入る。
   各部署は普段の入力画面で確かめて提出する。**提出済みの部署の値は上書きせず**、違いを一覧で出す
6. 外来の提出はこの画面の「照合して保存し、外来の日報を提出する」で行う

紙の余白に手書きしていた数字は、画面の下の「この日報から計算される値」に出る。

| 手書き | 計算 | 医事統計表① |
|---|---|---|
| 228 | 医科合計 − ドック − 健診 − 訪問診療科（266−18−18−2） | 外来患者数（`gairai_ippan`） |
| 303 | 医療介護合計 − 通所リハ（343−40） | 総合計より通所を除いた数（`total_ex_tsusho`） |
| 3,527 | 228 の今月累計 | 外来患者数の累計 |

病院日誌の外来（午前・午後・夜間・時間外１・時間外２）は医科合計の時間帯そのもの。
**時間外１＝00〜08、時間外２＝20〜24**（電子カルテの呼び方に合わせた）。

### 日報のExcelから読み込む

外来と入院は別々のExcelになっている。画面の2つの欄のどちらに選んでも、中身で種類を見分けて読む。
セル番地は決め打ちせず、見出しの文字（「科名」「医科合計」「病棟」「本日患者数」など）で位置を探すので、
様式に行や列が増えても読める（`src/nippo.php`）。

**外来日報（`20260925.xlsx` のような1日1ファイル）**

- **Sheet2** … 電子カルテから出した生データ（科 × 時間外１・午前診・午後診・夕診・時間外２ × 一般/救急 × 再/初/新）
- **Sheet1** … それを数式で並べ替えた紙の様式。リハビリ・ドック・健診・介護・訪問診療科内訳・
  健診科の新来と新患内訳は、医事課がここに手で入れている
- 読むのは **Sheet1（紙の様式）**。紙と同じ数字がすべてそろっているため（`nippo_gairai_from_xlsx()`）
- ファイルの「9月 3日」と画面の日付が違うときは読み込まない
- 表に無い科（特養科・精神科・透析科など）の行に人数が入っていたら、黙って捨てずに止める

**入院患者数日報（月1冊の `.xlsm`）**

- シート「1」〜「31」が日ごとの日報（ほかに MENU・目標・作業用）。**画面の日付と同じ日のシート** を読む（`nippo_nyuin_from_xlsx()`）
- 3F・4F・5F の前日患者数・入院・転入・退院・転出・本日患者数と、科別内訳を読む
- シートの「2026年9月」と日が、画面の日付と違うときは読み込まない（MENU の処理年月を確かめる）
- **まだ使っていない日のシートには、前の月の数字が残っている**（9月なのに「31」にも数字がある）。
  シートの日付は MENU から自動で入るので日付では見分けられない。そこで
  **前日のシートの本日患者数と、この日の前日患者数が合わなければ警告を出す**。警告が出たら紙と見比べる
- マクロ（.xlsm の中の VBA）は開かないし動かさない。値だけを読む

**共通**

- 読み込んだ値も、手で打った値と同じ照合（紙の合計と合うか、入退院の計算が合うか）を通ってから保存される
- 1つでも読めないファイルがあれば、どの欄も変えない
- 古い形式（.xls）は、Excelで「名前を付けて保存」→「Excel ブック（*.xlsx）」で保存し直す
- PHPの **zip 拡張** が要る。`php tools\check_env.php` の `zip / simplexml` が 注意 なら、
  `php.ini` の `extension=zip` の先頭の `;` を外して Apache を再起動する
- アップロードしたファイルはサーバに保存しない（読んだらすぐ消える一時ファイルのまま読む）

**すでに `seed_master.sql` を流したサーバでは**、この版で項目が増えたので流し直す
（実績データがまだ無ければ、流し直しても何も失わない）。

```
C:\php\php db\tools\build_seed.php
C:\php\php db\tools\run_sql.php db\seed_master.sql
```
...
外来の時間帯（`g1`〜`g5`、`g1n`〜`g5n`）と外来再掲リハビリ（`s1`）は、旧日誌に科別が無いため
「科別なし（旧日誌から移行）」の項目に入る。合計（病院日誌の外来欄・医事統計表）にはそのまま反映される。
ただし旧日誌にはドック・健診の **人数** が無い（`s11` `s12` は件数）ため、移行した日の
医事統計表①の外来患者数（`gairai_ippan`）はドック・健診を引けず、医科合計と同じ値になる。
