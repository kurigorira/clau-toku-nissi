...
def build_tejun():
    ...
    s.append(step('3.', 'テーブルとマスタを流し込む', [
        codebox(['php db\\tools\\run_sql.php db\\schema.sql --user=root --pass=＜rootのパスワード＞',
                 'php db\\tools\\build_seed.php                  ← 19部署 / 483項目 / 設定10件 を生成',
...
def build_report():
    ...
    s.append(table([
        ...
        ['作ったもの',
         ...
         '未入力の部署が一目で分かる'],
        ['規模', '19部署 / 483項目 / 画面14本。外部ライブラリなし・ビルド工程なし'],
        ['検証',
    ...
    p('このシステムの中身は、<b>項目マスタ（483行のCSV）</b> がほとんどすべてを決めている。'
      '入力画面も帳票も、このマスタから自動で組み立てられる。'
      '項目を増やすときに触るのはCSVの1行だけで、PHPは書き換えない。')
    tbl([
        ['区分', '件数', '意味'],
        ['入力', '396', '現場が手で入れる項目。ここだけがデータベースに保存される'
                        '（うち198は電子カルテ日報の科別×時間帯など）'],
        ['合算', '64', '「入院＋外来」のように、他の項目を足すだけの項目。入力欄は作らない'],
        ['計算', '23', '稼働率・平均在院日数・紹介率など、式で出す項目。入力欄は作らない'],
        ['<b>合計</b>', '<b>483</b>', '19部署ぶん'],
    ...
    tbl([
        ...
        ['外来 科別 × 時間帯（電子カルテ日報のとおり）', '日誌の「午前・午後・夜間・時間外」、統計表の「外来患者数」'],
    ...
    h2('2-3. 電子カルテの日報を転記してから集計する')
    p('外来と入院の人数は、電子カルテから毎日出る紙（「外来患者数（科別）」と「入院患者数日報」）が出発点になる。'
      '医事課はこの紙を見て、電卓で出した数字を余白に手書きし、Excelへ書き写していた。')
    p('新しい「日報転記」画面は、<b>紙と同じ並び</b>で科別×時間帯の人数を打ち込む。'
      '紙に印字されている医科合計・介護合計なども打ち、<b>科別の合計と合わなければ保存しない</b>。'
      '手書きしていた数字は、すべて計算で出る（9月24日の紙で確認）。')
    tbl([
        ['紙の手書き', '計算', '医事統計表①'],
        ['228', '医科合計 266 − ドック 18 − 健診 18 − 訪問診療科 2', '外来患者数'],
        ['303', '医療介護合計 343 − 通所リハ 40', '総合計より通所を除いた数'],
        ['3,527', '228 の今月累計', '外来患者数の累計'],
    ], [0.16, 0.56, 0.28], align={0: 'CENTER'})
    note('保存すると、病棟（入退院）と当直（訪問系・通所リハ）にも下書きとして値が入り、'
         '各部署が確かめて提出する。提出済みの部署の値は上書きしない。'
         '日報のExcel（.xlsx）があれば、画面で選ぶだけで外来の欄が埋まる（同じ照合を通ってから保存される）。')

    h2('2-4. 病床数のような「その時々で変わる値」')
    ...
    tbl([
        ...
        ['日報転記', '電子カルテの外来・入院日報を紙と同じ並びで転記。紙の合計と照合してから保存', '外来・医事課'],
    ...
    tbl([
        ...
        ['黄', '提出されたが、必須項目に空きがある（59項目が必須）', '確認する'],
    ...
    tbl([
        ...
        ['3', '外来の「時間外」が現行画面に2行ある。正しい区分名は', '<b>解決</b>：時間外１＝00〜08、時間外２＝20〜24'],
        ['4', '病院日誌「外来再掲」のリハビリの定義（Excelに対応する列がない）', '<b>解決</b>：電子カルテ日報のリハビリ行'],
        ['5', '時間外ウォークインの区分1〜4の意味', '「区分1」〜「区分4」のまま'],
        ['6', '外来患者数の定義（Excel①は「ドック・健診・訪問診察を除した数」）',
              '<b>解決</b>：日誌＝医科合計（266）、Excel①＝除いた数（228）'],
    ], [0.05, 0.55, 0.4])
    ...
    tbl([
        ...
        ['2. マスタの確認', '医事課に項目マスタ（483項目）と上記の残り3件を見ていただく'],
